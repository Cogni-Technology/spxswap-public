...
import { GetPortfolioResponse, type GetPortfolioRequest } from '@uniswap/client-data-api/dist/data/v1/api_pb'
...
export const getPortfolioQuery = <TSelectData = GetPortfolioResponse>({
  ...
  return queryOptions({
    ...
    subscribed: !!enabled,
    select,
    // SPXSwap: upstream DataApi is CORS-locked to app.uniswap.org. We don't
    // display user portfolio or balances in V1, so return an empty portfolio
    // response instead of undefined — React Query rejects undefined and the
    // downstream transforms need a real GetPortfolioResponse instance.
    queryFn: async (): Promise<GetPortfolioResponse> => new GetPortfolioResponse({ portfolio: undefined }),
  })
}

...
const swappableTokensQueryFn = (
  _params: SwappableTokensParams,
): QueryFunction<TradingApi.GetSwappableTokensResponse, QueryKey, never> | undefined => {
  // SPXSwap: upstream Trading API is CORS-locked. We don't support bridging in V1,
  // so we short-circuit to an empty token list instead of hitting the network.

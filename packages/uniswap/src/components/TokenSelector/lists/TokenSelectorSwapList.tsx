...
function useTokenSectionsForSwap({
  ...
  // SPXSwap: render common tokens as a flat vertical list (full-width rows)
  // instead of the upstream horizontal chip strip. Keeping SuggestedTokens as
  // the section key preserves the no-header rendering in OnchainItemList.web
  // (see the section.sectionKey !== OnchainItemSectionName.SuggestedTokens
  // branch), and flattening from TokenOption[][] → TokenOption[] makes the
  // renderItem route through TokenOptionItem instead of HorizontalTokenList.

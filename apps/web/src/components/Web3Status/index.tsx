...
function Web3StatusInner() {
  ...
  const isConnected = Boolean(activeAddresses.evmAddress || activeAddresses.svmAddress)
  ...
  }, [accountDrawer, isConnected, openConnectModal])
          ...
          {showLoadingState ? (
            ...
            <Flex key="normal" animation="125ms" enterStyle={{ opacity: 0, y: -2 }} exitStyle={{ opacity: 0, y: 2 }}>
              <Web3StatusGeneric
                ...
                loading={false}
                ref={ref}
                icon={<Wallet size={24} color="$accent1" />}
              >
                <AddressAndChevronContainer>
                  <Text variant="body2" marginRight={hasUnitag ? '$spacing8' : undefined}>
                    {accountIdentifier}

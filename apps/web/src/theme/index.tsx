...
export function ThemeProvider({ children, ...overriddenColors }: PropsWithChildren<Partial<ThemeColors>>) {
  ...
  // Mirror the selected theme onto <html data-theme> so global.css can style
  // the fixed #background-radial-gradient layer + html body-color against the
  // same light/dark state the Tamagui tokens use. Also persist a plain string
  // in localStorage so the inline <script> in index.html can pick up the
  // correct theme on the next page load BEFORE React mounts, avoiding the
  // pre-hydration dark flash when the user has selected light mode.
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem('spx-theme', themeName)
    } catch {

  ...
  return useCallback(
    (activity: ActivityUpdate) => {
      ...
      if (activity.type === ActivityUpdateTransactionType.BaseTransaction) {
        ...
        if (hash) {
          // SPXSwap: the original Transaction popup depends on an Apollo
          // `getTransactionToActivityQueryOptions` call that resolves against
          // the upstream activity backend we've stubbed out — the query
          // returns undefined and the popup renders null, so no toast is
          // visible. Fall back to a simple Success toast for swaps so the
          // user at least gets confirmation.
          const isSwapSuccess =
            original.typeInfo.type === TransactionType.Swap && update.status === TransactionStatus.Success
          const isSwapFailed =
            original.typeInfo.type === TransactionType.Swap && update.status !== TransactionStatus.Success
          if (isSwapSuccess) {
            popupRegistry.addPopup({ type: PopupType.Success, message: 'Swap confirmed' }, hash, popupDismissalTime)
          } else if (isSwapFailed) {
            popupRegistry.addPopup({ type: PopupType.Error, error: 'Swap failed' }, hash, popupDismissalTime)
          } else {

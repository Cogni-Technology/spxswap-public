...
function* handleSwapTransactionStep(params: HandleSwapStepParams): SagaGenerator<string> {
  ...
  if (!planId) {
    // SPXSwap: upstream uses PopupType.Transaction which resolves its content
    // via an Apollo activity query against the stubbed gateway — it returns
    // undefined and the popup renders null. Fire a visible Success toast with
    // a "Swap submitted" message immediately on broadcast so the user has
    // instant feedback, independent of when the downstream receipt poller
    // lands. The final "Swap confirmed" toast fires later from
    // state/activity/updater.tsx with a different key (the bare hash), so the
    // user sees: submitted → confirmed.
    popupRegistry.addPopup(
      { type: PopupType.Success, message: 'Swap submitted' },
      `${hash}-submitted`,

...
const DEPLOY_TARGET = process.env.DEPLOY_TARGET || 'ipfs'
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    // Relative asset paths so the build works from /ipfs/<cid>/ gateway URLs
    base: './',
    ...
    build: {
      outDir: 'build',
      sourcemap: VITE_DISABLE_SOURCEMAP || isProduction ? false : true,
